...
from util import Config, Dataset, Timer, load_dataset, get_num_cores, get_args, get_list_cores
...
def cache_by_sampling(data: Dataset) -> torch.Tensor:
    ...
    dataloader = dgl.dataloading.DataLoader(graph=data.graph, 
...
class CacheInstance:
    ...
    def __init__(self, cache_percentage: int, cache_priority: torch.Tensor, partition_mask: torch.Tensor):        
        ...
        self.num_p2p = torch.zeros((self.num_partition, self.num_partition), dtype=torch.int64)
    ...
    def update(self, input_nodes: torch.Tensor):
        device_pid = random.randint(0, self.num_partition - 1) # simulate: randomly selects a device to sample the mini batch
...
class CacheManager:
    ...
    def __init__(self, cache_policy : str, data : Dataset, partition_mask: torch.Tensor):
        ...
        while(cache_percentage <= 100):
            self.cache_instances.append(CacheInstance(cache_percentage, self.cache_priority, self.partition_mask))
...
def simulate_cache(config: Config, data: Dataset) -> CacheSimulator:
    ...
    partition_mask = torch.randint(0, config.num_partition, (data.graph.num_nodes(),), dtype=torch.int8)
...
def log_simulation(config: Config, data: Dataset, simu: CacheSimulator):
    ...
    with open(config.log_file, "w") as jsonfile:
        ret = dict()
